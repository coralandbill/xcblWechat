    ...
    <div class="new_list new_list1 t10">
        <ul id="newsUl">
            <script type="text/x-jquery-tmpl" id="newInfoTmpl">
                <li class="clearfix"><a href="/wechat/enforce/detail.do?id=#[$data.id]">#[$data.title]</a><span>#[$data.enforceDateFormat]</span></li>
            </script>
    ...
    function loadNewsInfo() {
        $.ajax({
            ...
            success: function (result) {
                if (result.success) {
                    loadingFlag = true;
                    var enforceList = result.enforceList;
                    if (enforceList != undefined && enforceList.length > 0) {
                        startPos += enforceList.length;
                    }
                    $("#newInfoTmpl").tmpl(enforceList).insertBefore("#news_p");
